...
namespace ctk
{
    ...
    class RunOnceThread : public SimpleThread
    {
        ...
        typedef void (T::*_F_on_run_once)();

        T* this_;
        _F_on_run_once fn_on_run_once_;

    public:
        RunOnceThread(T* _this,
                    _F_on_run_once _fn_on_run_once, 
                    pcstr _name)
            : _Super(0, _name)
            , this_(_this)
    ...
    template<class C>
    ctk::Handle<SimpleThread>
    runInThread(C* _this, void (C::*_fn)(), pcstr _name, bool _createNewThread=true)
    {
        if (_createNewThread)
        {
            ctk::Handle<SimpleThread> thr = ctk::allocate<RunOnceThread<C> >(CTK_CURRENT, _this, _fn, _name);
            thr->start();
            return thr;
        }
        else
