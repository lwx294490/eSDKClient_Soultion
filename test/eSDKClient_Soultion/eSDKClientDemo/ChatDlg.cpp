	...
	if (m_convId.empty())	//如果当前不存在会话，则需要添加AccountList
	{
		...
		if (IsMeetingConv())
		{
			for (int i=0; i<m_ConfMemList.GetItemCount(); i++)
			{
				...
				if (szAccTemp == m_szAccount)	//AccountList不包含自己
				{ 
					continue;
				}
...
{
	// TODO: 在此添加控件通知处理程序代码
	//conference call	会议呼叫（前提先MakeMeetingGroup）
	if (IsMeetingConv())
	{
